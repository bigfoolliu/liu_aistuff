...
* [x.面试](#x.面试)
    ...
    - [x.13mysql主从复制原理](#x.13mysql主从复制原理)
...
1. sql语句结果缓存，条件苛刻，不可控
2. innodb buffer pool缓存，缓存的磁盘上的分页数据，不是sql查询结果
...
- [简书:mysql主从复制](https://www.jianshu.com/p/faf0127f1cb2)
- 概念: 也称为AB复制，允许将数据从主mysql数据库服务器复制到一个或者多个从mysql服务器(可以是一张表，也可以是所有的数据库)
- 复制是异步的， 从服务器不需要和主服务器保持永久连接

优点：

- `横向扩展方案`, 主服务器用来写更新，多个从用来读，可以用来负载提高性能
- `数据安全性`, 从站的备份数据不影响主站
- `分析`，主服务器创建数据，从服务器分析，不影响主服务器性能
- `远程数据分发`, 可以使用复制为远程站点创建数据的本地副本，而无需永久访问主服务器

原理(步骤)：

1. 主服务器上的任何修改通过I/O thread保存在二进制日志`Binary log`中
2. 从服务器启动一个I/O thread,配置账号和密码，连接到主服务器，请求求二进制文件，将读取到的二进制文件写到本地的`中继日志(Realy log)`中
3. 从服务器开启SQL thread定时检查中继日志，如果发现更新则把更新的内容在本机的数据库执行一遍
