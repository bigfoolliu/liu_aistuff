...
- 简单
- 内存有限，处理能力有限
- ![单机结构](./img/../imgs/redis_single_node.png)

### 7.2主从复制

- ![主从复制结构](./imgs/redis_master_slave.png)

### 7.3哨兵模式

- ![哨兵结构](./imgs/redis_sentinel.png)
...
- ![集群proxy型](./imgs/redis_sentinel_proxy.png)
...
导致集群方案不可用的情况： 若有A B C三个节点，但是没有复制模型的情况下，B失效，会导致集群缺失一部分数据。
...
- ![集群直连型](./imgs/redis_sentinel_direct.png)
