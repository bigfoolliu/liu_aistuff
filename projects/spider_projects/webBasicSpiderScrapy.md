...
* [4. HTTP基本原理](#4.-http基本原理)
        - [4.1 URI和URL](#4.1-uri和url)
        - [4.2 超文本](#4.2-超文本)
        - [4.3 HTTP和HTTPS](#4.3-http和https)
        - [4.4 HTTP请求过程](#4.4-http请求过程)
        - [4.5 请求](#4.5-请求)
                + [4.5.1 请求方法](#4.5.1-请求方法)
                + [4.5.2 请求网址](#4.5.2-请求网址)
                + [4.5.3 请求头](#4.5.3-请求头)
                + [4.5.4 请求体](#4.5.4-请求体)
                + [4.5.5 响应](#4.5.5-响应)
                        * [4.5.5.1 响应状态码](#4.5.5.1-响应状态码)
                        * [4.5.5.2 响应头](#4.5.5.2-响应头)
                        * [4.5.5.3 响应体](#4.5.5.3-响应体)
* [5. 网页基础](#5.-网页基础)
        - [5.1 网页的组成](#5.1-网页的组成)
                + [5.1.1 HTML](#5.1.1-html)
                + [5.1.2 CSS](#5.1.2-css)
                + [5.1.3 JavaScript](#5.1.3-javascript)
        - [5.2 网页的结构](#5.2-网页的结构)
        - [5.3 节点树及节点间的关系](#5.3-节点树及节点间的关系)
        - [5.4 选择器](#5.4-选择器)
* [6. 爬虫的基本原理](#6.-爬虫的基本原理)
        - [6.1 爬虫概述](#6.1-爬虫概述)
                + [6.1.1 获取网页](#6.1.1-获取网页)
                + [6.1.2 提取信息](#6.1.2-提取信息)
                + [6.1.3 保存数据](#6.1.3-保存数据)
        - [6.2 可抓取数据类型](#6.2-可抓取数据类型)
        - [6.3 JavaScript渲染页面](#6.3-javascript渲染页面)
* [7. 会话及cookie](#7.-会话及cookie)
        - [7.1 静态网页及动态网页](#7.1-静态网页及动态网页)
        - [7.2 无状态HTTP](#7.2-无状态http)
                + [7.2.1 会话](#7.2.1-会话)
                + [7.2.2 Cookies](#7.2.2-cookies)
                + [7.2.3 会话维持](#7.2.3-会话维持)
* [8. 代理](#8.-代理)
        - [8.1 代理的基本原理](#8.1-代理的基本原理)
        - [8.2 代理的作用](#8.2-代理的作用)
        - [8.3 代理的分类](#8.3-代理的分类)
        - [8.4 常见代理设置](#8.4-常见代理设置)
* [9. 基本库的使用](#9.-基本库的使用)
        - [9.1 urlib库](#9.1-urlib库)
        - [9.2 requests库](#9.2-requests库)
        - [9.3 正则表达式(Regular Expression),RE](#9.3-正则表达式(regular-expression),re)
        - [9.4 猫眼Top100电影信息抓取](#9.4-猫眼top100电影信息抓取)
* [10. 解析数据之解析库的使用](#10.-解析数据之解析库的使用)
        - [10.1 使用XPath](#10.1-使用xpath)
        - [10.2 使用Beautiful Soup](#10.2-使用beautiful-soup)
        - [10.3 使用pyquery](#10.3-使用pyquery)
* [11. 数据存储](#11.-数据存储)
        - [11.1 TXT文本存储](#11.1-txt文本存储)
        - [11.2 JSON文件存储](#11.2-json文件存储)
                + [11.2.1 对象和数组](#11.2.1-对象和数组)
                + [11.2.2 读取JSON](#11.2.2-读取json)
        - [11.3 CSV文件存储](#11.3-csv文件存储)
* [11.4 关系型数据库存储](#11.4-关系型数据库存储)
        - [11.4.1 MySQL存储](#11.4.1-mysql存储)
* [11.5 非关系型数据存储](#11.5-非关系型数据存储)
        - [11.5.1 MongoDB存储](#11.5.1-mongodb存储)
        - [11.5.2 Redis存储](#11.5.2-redis存储)
* [12. Ajax数据爬取](#12.-ajax数据爬取)
* [13. 动态渲染页面爬取](#13.-动态渲染页面爬取)
        - [13.1 Selenium使用](#13.1-selenium使用)
        - [13.2 Splash使用](#13.2-splash使用)
