...
- [mysql面试](#mysql面试)
    - [1.mysql基础使用及命令](#1mysql基础使用及命令)
    - [2.索引相关](#2索引相关)
        - [2.1基础概念](#21基础概念)
        - [2.2mysql索引类型](#22mysql索引类型)
            - [2.2.1哈希索引](#221哈希索引)
            - [2.2.2B+树索引](#222b树索引)
        - [2.3建立索引考虑要素](#23建立索引考虑要素)
        - [2.4联合索引](#24联合索引)
        - [2.5判断索引是否被使用](#25判断索引是否被使用)
        - [2.6查询不会使用索引的情况](#26查询不会使用索引的情况)
    - [3.事务相关](#3事务相关)
        - [3.1事务产生原因](#31事务产生原因)
            - [3.1.1锁](#311锁)
            - [3.1.2锁的类型](#312锁的类型)
    ...
    - [4.表结构设计](#4表结构设计)
        - [4.1主键](#41主键)
        - [4.2NULL与NOT NULL](#42null与not-null)
        - [4.3用户密码散列用什么类型存储](#43用户密码散列用什么类型存储)
    - [5.存储引擎相关](#5存储引擎相关)
        - [5.1mysql支持哪些引擎及其区别](#51mysql支持哪些引擎及其区别)
    - [6.零散](#6零散)
        - [6.1char与varchar的区别](#61char与varchar的区别)
        - [6.2varchar(10)与int(10)的区别](#62varchar10与int10的区别)
        - [6.3binlog的录入方式与区别](#63binlog的录入方式与区别)
        - [6.4超大分页的处理](#64超大分页的处理)
        - [6.5业务中sql的耗时以及慢查询优化](#65业务中sql的耗时以及慢查询优化)
        - [6.6横向分表和纵向分表的例子](#66横向分表和纵向分表的例子)
        - [6.7什么是存储过程及其优缺点](#67什么是存储过程及其优缺点)
        - [6.8三个范式](#68三个范式)
        - [6.9mybatis中的#与$的区别](#69mybatis中的与的区别)
...
底层实现是多路平衡查找树，每次查找从根节点出发，到叶子节点得到键值，找到记录。
...
即锁作用的范围，因为数据的读取和写入如果不是同一个数据的时候，频繁加锁会影响性能。
...
两个操作占有自己的资源不释放，然后请求对方的资源造成。

解决方式：**Innodb将持有最少行数排他锁的事务进行回滚来打破。**
...
1. `未提交读`，使用较少，即`未提交就可以读`，其他事务可以看到本事务没有提交的更改，可能会造成`脏读`
